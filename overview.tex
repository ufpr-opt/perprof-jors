...
\subsection*{Motivation}
    ...
    There are  third part implementations to generate PP. The same group that created PP  released a MATLAB script in their website~\cite{url:cops}. There is also a module written by Michael Friedlander inside NLPy~\cite{url:NLPpy} that is able to produce PP. However, there are features that some users need it and those softwares did not delivered.

    We thus designed   a straightforward  tool that would  allow one to create   performance profile pictures in a fast and easy manner.

    In addition,  this tool  would allow \LaTeX\ users, a group in which almost all    optimization community is included,  to   generate performance profile
